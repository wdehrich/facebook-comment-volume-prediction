...
corrs = c(rep(0,6))
best = c(rep(0,52))
for( n in c(1:24,29:33,35:52) ){
  corrs[1] = cor(fb[,n]^3,fb[,53]) # cubic
  corrs[2] = cor(fb[,n]^2,fb[,53]) # square
  corrs[3] = cor(fb[,n],fb[,53]) # constant
  corrs[4] = cor(sqrt(fb[,n]),fb[,53]) # square root
  corrs[5] = cor(log(fb[,n]+1),fb[,53]) # log
  corrs[6] = cor(1/(fb[,n]+1),fb[,53]) # reciprocal
  cand = 0
  for( i in 1:6 ){
    if( abs(corrs[i]) > cand ){
      cand = corrs[i]
...
# write to file
write.csv(fb_new, "E:/facebook_transformed.csv", row.names=F)
